...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (CurrentGameState == GameState.GameOver) {
            if (UIManager.Instance != null && RoundManager.Instance == null) {
                // ラウンドシステムが無効な場合のみゲームオーバー表示
                UIManager.Instance.ShowGameOver(GetWinnerName());
            }
        }
    }
    ...
    private void UpdateUI() {
        // UIManagerを使用してプレイヤーHP表示を更新
        if (UIManager.Instance != null) {
            // プレイヤー1のHP表示更新
            if (player1_ != null) {
                string player1Id = player1_.playerID_;
                if (playerCurrentHp_.ContainsKey(player1Id)) {
                    UIManager.Instance.UpdatePlayerHP(player1Id, playerCurrentHp_[player1Id], playerMaxHp_[player1Id], player1Name_);
                }
            }
            ...
            if (player2_ != null) {
                ...
                if (playerCurrentHp_.ContainsKey(player2Id)) {
                    UIManager.Instance.UpdatePlayerHP(player2Id, playerCurrentHp_[player2Id], playerMaxHp_[player2Id], player2Name_);
                }
            }
        }
    }
